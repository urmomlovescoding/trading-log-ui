...
export const handler = async (event = {}) => {
  ...
  if (method === "GET") {
    return {
      statusCode: 200,
      headers: corsHeaders(),
      body: JSON.stringify({ ok: true, message: "POST a trade to this endpoint." })
    };
  }
  if (method !== "POST") {
  ...
  // Accept either PK/SK or acctId/sortKey from the client
  ...
  if (missing.length) {
    return { statusCode: 400, headers: corsHeaders(), body: JSON.stringify({ error: "Missing fields", missing }) };
  }
